...
desenha_stati proc near
	...
	final_desenho_asterisco:	; Caso aconteça de ter
		...
		int 10h
		jmp final_desenho
	escreve_ultimo:				; arruma a tela caso ela suba
		mov tam_7,cl
		mov ah,0Ah
		mov al,t7
		mov cx,1
...
conta_palavras proc near		; sempre inicializo a variavel aux_tam com 0 e começo a incrementar ela 
								; enquanto não houver um espaço, tab, ou troca de linha, quando houver eu vejo qual o tamanho dela e incremente na variavel do respectivo tamanho
		lea	bx,buffer_arq		; quando é a ultima palavr a("0") vai para uma nova função para terminar após as comparações
	conta_palavras_in:
		mov aux_tam,0
	inicio_contagem:
		mov cl,[bx]
...
;###################################################
le_proxima_linha PROC NEAR		; recebe o arquivo e vai lendo linha por linha (a variação é para saber a partir de onde a próxima função tem que ler)
	mov tamlimpeza,80			
	lea	ax,linhas_escritas
	call limpa_linhas
	lea	si,buffer_arq
